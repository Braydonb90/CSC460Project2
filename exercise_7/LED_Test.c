...
void init_LED_D2(void){
	...
	DDRH|= LED_D2;		//Set LED to output (pins 4 and 5)
	PORTH = 0x00;		//Initialize port to LOW (turn off LEDs)
}

void init_LED_D5(void)
{
	DDRH |= LED_D5;		//Set LED to output (pins 6 and 7)
	PORTH = 0x00;		//Initialize port to LOW (turn off LEDs)
}

void enable_LED(unsigned int mask)
{
	PORTH = mask;		//Initialize port to high
}

void disable_LEDs(void)
{
	PORTH = 0x00;	//Initialize port to high
}
